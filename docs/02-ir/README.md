...
但是这里需要注意一下的是，IR不一定非得是3地址码。2地址码的表达能力也是完全的，比如我们现在最常用的硬件架构x86的汇编指令集就是2地址码的形式，例如 `add r1, r2` ，最终结果存 $r_1$ 。只不过2地址码的指令集会比3地址码更大，因为三地址码的 `r3 = r1 + r2` 可能需要二地址码的 `mov r3, r1` ， `add r3, r2` 两条指令来表示。

不过如果单纯只是为了分析的话，我们一般会更青睐3地址码，指令集更小，表达能力也足够。
